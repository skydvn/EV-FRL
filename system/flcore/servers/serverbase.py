...
class Server(object):
    ...
    def cos_sim_2grad(self, grad1, grad2):
        params1 = torch.cat([p.data.view(-1) for p in grad1.parameters()])
        params2 = torch.cat([p.data.view(-1) for p in grad2.parameters()])
    ...
    def save_client_model(self, round):
        ...
        if not os.path.exists(model_path):
            os.makedirs(model_path)
        for client in self.clients:
            client_model_path = os.path.join(model_path, self.algorithm + "_client" + str(client.id) + "_round" + str(
                round) + ".pt")
